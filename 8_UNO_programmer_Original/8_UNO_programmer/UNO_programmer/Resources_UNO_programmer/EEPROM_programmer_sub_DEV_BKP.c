...
char Download_text(int *,int *,char *,int *,  unsigned int *);
char Download_data(int *,int *,char *,int *,int *, unsigned int*);
...
char Read_EEPROM_buffer(int, unsigned int*);
void Write_EEPROM_buffer(int, unsigned int*, char);


int RBL = 399;				//255;	


char Read_EEPROM_buffer(int array_offset, unsigned int EEPROM_buffer[]){
int temp;
temp = array_offset;
if (!((temp)%2))return (EEPROM_buffer[temp/2] >> 8);
else return EEPROM_buffer[temp/2];}



void Write_EEPROM_buffer(int array_offset, unsigned int EEPROM_buffer[], char text){
int temp;
temp = array_offset;
if (!((temp)%2))(EEPROM_buffer[temp/2] = text);
else {EEPROM_buffer[temp/2] = (EEPROM_buffer[temp/2] << 8) + text;}}
...
/********************************************************************************************************************************************/
void Prog_EEPROM(void){	
unsigned int  EEPROM_buffer[200];		//256			
...
do 																		//Remain in do-loop until text all downloaded
	...
	if (!(DL_status))													//If buffer full and space available in EEPROM
		...
		//for(int m =0; m<=127; m++){sendChar(EEPROM_buffer[m] >>8);sendChar(EEPROM_buffer[m]);}
...
char *ptr_DL_flag,\
int *ptr_array_pointer, unsigned int EEPROM_buffer[]){			//int *ptr_array_pointer
int UART_counter = 0;
char text_char;	//text_char_old, keypress;
//int array_pointer_local;
...
if (text_char != '"')													//Check for -"-? No: Enter Loop 2: Yes: skip loop 2.
	...
	//array_pointer_local = 	(*ptr_array_pointer)-1;	
				
				if(Read_EEPROM_buffer((*ptr_array_pointer)-1, EEPROM_buffer)	 != '\0')
				
				//if(Read_EEPROM_buffer((array_pointer_local), EEPROM_buffer)	 != '\0')		
	...
	//array_pointer_local = 	(*ptr_array_pointer)++;	
		
		Write_EEPROM_buffer((*ptr_array_pointer)++,EEPROM_buffer, text_char);
		//Write_EEPROM_buffer(array_pointer_local,EEPROM_buffer, text_char);
...
if (*ptr_DL_flag == 1)													//If EEPROM buffer is full: Text download is not complete													
	...
	*ptr_file_pointer = UART_counter;									//Save the "UART_counter" in "file_pointer"
	...
	//Write_EEPROM_buffer((*ptr_array_pointer)++,EEPROM_buffer, '\0');
	//array_pointer_local = 	(*ptr_array_pointer)++;
	//Write_EEPROM_buffer(array_pointer_local,EEPROM_buffer, '\0');
	...
	while (((*ptr_EEP_pointer) +n) < EE_top)							//Stop copying when space reserved for the application is reached
		...
		Read_EEPROM_buffer(n, EEPROM_buffer));
		//EEPROM_buffer[n]);		
	...
	if((*ptr_EEP_pointer) >= EE_top)									//If saving the entire string would cause the EEPROM allocation to be exceeded
		{*ptr_DL_flag = 3; 												//set the download flag to 3
			...
			< (EE_top-1))
				...
				(*ptr_array_pointer) + n),\
				Read_EEPROM_buffer(n, EEPROM_buffer));
				//EEPROM_buffer[n]);										//Copy as much as possible of the EEPROM buffer to the EPPROM
...
while((*ptr_DL_flag ==0) || (*ptr_DL_flag ==2)) 						//EEPROM_bufer contains end of text plus data (if any) or only contains data
	...
	case '4': case '5':  case '6': case '7': case '8': case '9':  
		...
		//array_pointer_local = (*ptr_array_pointer)++;
		//Write_EEPROM_buffer(array_pointer_local, EEPROM_buffer, (data_int >> 8));
		//EEPROM_buffer[(*ptr_array_pointer)++] = (data_int >> 8);		//save each number in EEPROM buffer as integer
		Write_EEPROM_buffer((*ptr_array_pointer)++, EEPROM_buffer, (data_int >> 8));
		
		
		//array_pointer_local = (*ptr_array_pointer)++;
		//Write_EEPROM_buffer(array_pointer_local, EEPROM_buffer, (data_int & 0x00FF));
		//EEPROM_buffer[(*ptr_array_pointer)++] = (data_int & 0x00FF);
		Write_EEPROM_buffer((*ptr_array_pointer)++, EEPROM_buffer, (data_int & 0x00FF));
