...
/*SNOW STORM DISPLAY: 
Uses the watch dog timer to prevent the display from getting jammed in a way that can sometimes 
affect programs.
...
Project subroutine "PRN_16bit_GEN_UNO (0)" uses a number stored in EEPROM to generate another one 
which it saves in the same EEPROM location. In this way repeated calls to "PRN_16bit_GEN_UNO (0)" 
generate a sequence of random numbers.

Note: 
a.  EEPROM memory is not affected by POR or a WD timeout.
b.  "PRN_16bit_GEN_UNO (Num)" generates a random number based on "Num" and it is up to the Firm Ware
...
4.  SW_3 also controls the intensity of the display.  The "setup_UNO" macro tests switch_2
