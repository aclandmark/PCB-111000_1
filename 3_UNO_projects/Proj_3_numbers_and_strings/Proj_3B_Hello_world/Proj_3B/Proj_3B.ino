...
int main (void){
const char * message_ptr[4];                                      //Define an array that will be used to hold the addresses of the data 
                                                                  //to be processed (i.e. printed out in this case)


                                                                  //Four text strings saved to program memory
const char *message_1 = "Hello world";                            //Note the alternative ways of defining these text strings    
const char *message_2 = "Sending text to a PC";                   //The '*' tells the compiler that the name "message_1" is a pointer 
                                                                  //It gives the address of the first letter in the string
                              
const char message_3[] = "Writing C programs";                    //Defines array called "message_3" and loads it with the string
const char message_4[] = "Downloading them to an ATMEGA device";  //The name "message_3" is also a pointer. It gives the address of the 'W'.
...
Char_to_PC('\t'); Char_to_PC('\t');                               //Send a tab
...
void String_to_PC_local(const char s[]){                        //Local function only, normally use "String_to_PC()"            
...
void String_to_PC_alt(const char* s){                          //Note alternative notations based on pointers            
int i=0;                          
while(1){
if(*(s + i) == '\0') break;               
Char_to_PC(*(s + i));                      
