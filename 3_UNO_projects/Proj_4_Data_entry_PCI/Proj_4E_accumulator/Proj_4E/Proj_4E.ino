...
press sw2 rather than entering new data to reset everything
...
while(1){if ((switch_2_down) && (!(no_reset)))              //clear accumulator and reset
{I2C_Tx_accumulator_op(digits, 'C');SW_reset;}              //only allowed when new number is zero  

if(!(Return_key)); else {no_reset = 0; break;}}             //Line A wait here for data to be entered using sw3 and sw3_isr
Return_key=0;cli();                                         //Clear interrupts so that sw3 can be used for other purposes

while(1){ 
while ((switch_1_up) && (switch_2_up));                     //wait for switch_1 or 2 to be pressed.
if (switch_3_down)break;                                    //If switch_3 down exit and reinitialise new number

if(switch_1_down)I2C_Tx_accumulator_op(digits, 'A');        //Add new number to accumulator

if(switch_2_down)I2C_Tx_accumulator_op(digits, 'S');        //Subtract new number from accumulator
while((switch_1_down) || (switch_2_down));                  //wait for switch_1 or 2 to be released (inside while loop)
Timer_T0_10mS_delay_x_m(5);}                                //switch bounce delay

while((switch_1_down) || (switch_2_down));                  //wait for switch_1 or 2 to be released (outside while loop)
...
/****************************************************************************************************************/
ISR(PCINT0_vect){accumulator_sw3_isr();}
ISR(PCINT2_vect){accumulator_sw1_and_sw2_isr();}




/****************************************************************************************************************/
void accumulator_sw3_isr(void){     
Timer_T0_10mS_delay_x_m(5);                               //sw bounce delay
clear_PCI_on_sw3;                                         //clear spurious interrupts
...
/****************************************************************************************************************/
void accumulator_sw1_and_sw2_isr(void){

Timer_T0_10mS_delay_x_m(5);                             //switch bounce delay
clear_PCI_on_sw1_and_sw2;                               //clear spurious interrupts
